...
|---|---|---|---|
| **S1** | Auth gate (sidebar deferred to S3) | /app and /app/* redirect to /auth/login when anonymous. Logged-in users see existing /app exactly as today — NO UI change yet. Sidebar moved to S3 where it has a real CRM home to wrap. Keeps S1 surgical + low-risk. | Anonymous: /app → /auth/login. Logged in: /app unchanged from today. |
