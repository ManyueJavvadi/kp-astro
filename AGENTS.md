# DevAstroAI — Project notes for Codex

This file is loaded by Codex at every session start. It captures
...
For the *evolving* plan of record, read `.Codex/BACKLOG.md` (Track A
queue, business model, Option A stance) and `.Codex/DAILY_LOG.md`
...
- `.Codex/research/world-first-vision.md` — **CANCELLED**. 8-workspace
...
- `.Codex/research/v2-roadmap.md` — **SUPERSEDED** (earlier still).
- `.Codex/research/phase-a-handoff.md` — **COMPLETE**. The Phase A
...
**Audit doc**: `.Codex/research/analysis-deep-audit.md` (PR A1.9) — every
...
- **Preserved v2 reference**: `.Codex/worktrees/v2-phase0-design/` — the
...
| PR24 | (reverted) Restore-only-on-reload gate + this AGENTS.md |
| PR25 | Reverted the localStorage session auto-restore entirely; kept masked-input helpers + AGENTS.md + synced match houses + match mobile grid |
...
- Full scope + process + PR queue documented in `.Codex/BACKLOG.md` under `## Track A.1 — Backend KP accuracy audit`.
- Research docs live in `.Codex/research/{tool}-audit.md` (created per-tool; directory does not exist yet — create it with the first audit).
...
Phase 15 docs:
- `.Codex/DAILY_LOG.md` 2026-05-10 entry — full PR-by-PR breakdown
...
Shipped 2026-05-07 on `Codex/eager-elbakyan`. A 14-section deterministic
...
These tripped Phase 14 three times — document them so future Codex
doesn't re-discover:
...
24h re-enable is on the Hot backlog — see `.Codex/BACKLOG.md`.
