      ...
      {houseOrder.map((house, idx) => {
        ...
        if (house === 0) return (
          <div key={idx} style={{
            ...
            {isCenter && idx === 5 && (
              <span style={{ color: "rgba(201,169,110,0.15)", fontSize: 20, fontFamily: "'DM Serif Display',serif" }}>✦</span>
            )}
          </div>
        );
        ...
        return (
          <div key={idx}
            style={{
              background: isLagna ? "rgba(201,169,110,0.07)" : "rgba(201,169,110,0.015)",
              border: `0.5px solid ${isLagna ? "rgba(201,169,110,0.3)" : "rgba(201,169,110,0.12)"}`,
              padding: "4px 5px", minHeight: 95,
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
              <span style={{ fontSize: 8, color: isLagna ? "var(--accent)" : "rgba(201,169,110,0.45)", fontWeight: isLagna ? 700 : 400 }}>
                {house}{isLagna ? "↑" : ""}
              </span>
              <span style={{ fontSize: 7, color: "rgba(201,169,110,0.3)" }}>
                {cusp?.degree_in_sign?.toFixed(0)}°
              </span>
            </div>
            {/* Sign */}
            <div style={{ fontSize: 7, color: "rgba(201,169,110,0.35)", marginBottom: 3, lineHeight: 1.2 }}>
              {cusp?.sign_te?.slice(0, 4) || ""}
            </div>
            {/* Planets */}
            <div style={{ display: "flex", flexDirection: "column", gap: 2, flex: 1 }}>
              {hp.map((p: any) => (
                <div key={p.planet_en}
                  title={`${p.planet_en} | ${p.nakshatra_en} | ${p.degree_in_sign.toFixed(1)}° | Star: ${p.star_lord_en} | Sub: ${p.sub_lord_en}`}
                  ...
                  <span style={{
                    ...
                    {p.planet_short}{p.retrograde ? "℞" : ""}
                  </span>
                  <span style={{ fontSize: 7, color: `${PLANET_COLORS[p.planet_en]}80` || "#444" }}>
                    {p.degree_in_sign.toFixed(0)}°
                  </span>
                </div>
              ))}
            </div>
  ...
  return (
    <div style={{ background: "var(--surface2)", border: "0.5px solid var(--border)", borderRadius: 10, padding: "1rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        {[
          { l: "వారం", v: data.vara, sub: data.vara_en },
          { l: "తిథి", v: data.tithi, sub: `#${data.tithi_num}` },
          { l: "నక్షత్రం", v: data.nakshatra, sub: data.nakshatra_en },
          { l: "యోగం", v: data.yoga, sub: data.yoga_en },
        ].map(item => (
...
export default function Home() {
  ...
  const handleAsk = async () => {
    ...
    setQuestion("");
    try {
      const res = await axios.post(`${API_URL}/prediction/ask`, { name: birthDetails.name, date: formattedDate, time: getTime24(), latitude: birthDetails.latitude, longitude: birthDetails.longitude, timezone_offset: 5.5, topic: "auto", question: currentQuestion, mode: "user", history: messages.slice(-4).map(m => ({ question: m.question, answer: m.answer })) });
  ...
  const houseNames: Record<number, string> = { 1: "Self", 2: "Wealth", 3: "Siblings", 4: "Home", 5: "Children", 6: "Health", 7: "Marriage", 8: "Longevity", 9: "Luck", 10: "Career", 11: "Gains", 12: "Losses" };
  ...
  const TABS = [
    ...
    { id: "dasha", label: "దశ", en: "Dasha" }, { id: "ruling", label: "రూలింగ్", en: "Ruling Planets" },
  ...
  const resetAll = () => {
    ...
    setActiveTopic(""); setActiveTab("chart");
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", fontFamily: "'Noto Sans Telugu','DM Sans',sans-serif" }}>
      <style>{`
        ...
        .md-body blockquote{border-left:2px solid var(--accent);padding-left:.75rem;color:var(--muted);font-style:italic;margin:.5rem 0}
      ...
      <div style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none" }}>
        {[...Array(90)].map((_, i) => (
          <div key={i} style={{ position: "absolute", width: i % 15 === 0 ? "2px" : "1px", height: i % 15 === 0 ? "2px" : "1px", background: "white", borderRadius: "50%", left: `${(i * 137.5) % 100}%`, top: `${(i * 97.3) % 100}%`, opacity: 0.08 + (i % 5) * 0.07 }} />
        ))}
      </div>
      ...
      {setupDone && mode === "astrologer" && workspaceData && (
        <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative", zIndex: 5 }}>
          ...
          {/* Sidebar */}
          <div style={{ width: 210, borderRight: "0.5px solid var(--border)", background: "var(--surface)", display: "flex", flexDirection: "column", overflow: "hidden", flexShrink: 0 }}>
            ...
            <div style={{ padding: "0.75rem 1rem", borderBottom: "0.5px solid var(--border)" }}>
              <div style={{ fontSize: 9, color: "var(--accent)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 6 }}>పంచాంగం · ఇప్పుడు</div>
            ...
            <div style={{ padding: "0.75rem 1rem", borderBottom: "0.5px solid var(--border)" }}>
              <div style={{ fontSize: 9, color: "var(--accent)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 6 }}>దశ</div>
            ...
            <div style={{ padding: "0.75rem 1rem", flex: 1, overflow: "auto" }}>
              <div style={{ fontSize: 9, color: "var(--accent)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 6 }}>రూలింగ్ గ్రహాలు</div>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {/* Tabs */}
            <div style={{ display: "flex", borderBottom: "0.5px solid var(--border)", background: "var(--surface)", overflowX: "auto", flexShrink: 0 }}>
              {TABS.map(tab => (
            ...
            <div style={{ flex: 1, overflow: "auto", padding: "1.25rem" }}>
              ...
              {/* CHART */}
              {activeTab === "chart" && (
                <div className="tab-content" style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: "1.5rem", alignItems: "start" }}>
                  <div>
                    <div style={{ fontSize: 10, color: "var(--accent)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "0.6rem" }}>దక్షిణ భారత చార్ట్</div>
                    <SouthIndianChart planets={workspaceData.planets} cusps={workspaceData.cusps} />
                    <div style={{ fontSize: 9, color: "var(--muted)", marginTop: 6, textAlign: "center" }}>Hover over house for details · ↑ = Lagna</div>
                  </div>
                  <div>
                    <div style={{ fontSize: 10, color: "var(--accent)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "0.75rem" }}>గ్రహ స్థానాలు</div>
                    {workspaceData.planets.map((p: any) => (
                      <div key={p.planet_en} style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 8px", background: "var(--surface2)", borderRadius: 6, border: "0.5px solid var(--border)", marginBottom: 3 }}>
                        ...
                        <span style={{ fontSize: 10, color: "var(--muted)", minWidth: 30 }}>H{p.house}</span>
  ...
  <div className="tab-content">
    <div style={{ fontSize: 10, color: "var(--accent)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "0.75rem" }}>గ్రహ స్థాన పట్టిక · KP పద్ధతి</div>
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
      <thead>
        <tr>
          {["గ్రహం", "భావం", "రాశి", "అంశం", "నక్షత్రం", "నక్షత్రాధిపతి", "సబ్ లార్డ్"].map(h => (
            <th key={h} style={{ textAlign: "left", padding: "8px 10px", color: "var(--muted)", fontSize: 10, letterSpacing: "0.06em", borderBottom: "0.5px solid var(--border)", fontWeight: 400, whiteSpace: "nowrap" }}>{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>
        {workspaceData.planets.map((p: any) => (
          <tr key={p.planet_en} style={{ borderBottom: "0.5px solid rgba(201,169,110,.06)" }}
            onMouseEnter={e => (e.currentTarget.style.background = "var(--surface2)")}
            onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
            <td style={{ padding: "9px 10px", color: PLANET_COLORS[p.planet_en], fontWeight: 600, whiteSpace: "nowrap" }}>
              {p.planet_te} {p.retrograde && <span style={{ fontSize: 9, color: "var(--red)" }}>℞</span>}
            </td>
            <td style={{ padding: "9px 10px" }}>
              <span style={{ background: "rgba(201,169,110,0.1)", color: "var(--accent)", border: "0.5px solid var(--border2)", borderRadius: 4, padding: "2px 8px", fontSize: 11, fontWeight: 600 }}>H{p.house}</span>
            </td>
            ...
            <td style={{ padding: "9px 10px" }}>
              <span style={{ background: "rgba(201,169,110,.1)", color: "var(--accent)", border: "0.5px solid var(--border2)", borderRadius: 4, padding: "2px 8px", fontSize: 11 }}>{p.sub_lord_te}</span>
            </td>
          </tr>
        ))}
      </tbody>
              ...
              {activeTab === "cusps" && (
                <div className="tab-content">
                  <div style={{ fontSize: 10, color: "var(--accent)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "0.75rem" }}>భావ కస్పాల పట్టిక · KP కీలకం</div>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              ...
              {activeTab === "significators" && (
                <div className="tab-content">
                  <div style={{ fontSize: 10, color: "var(--accent)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "0.75rem" }}>కారక గ్రహాలు · అన్ని భావాలు</div>
              ...
              {activeTab === "dasha" && (
                <div className="tab-content">
                  <div style={{ fontSize: 10, color: "var(--accent)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "1rem" }}>దశ కాలరేఖ · {workspaceData.mahadasha.lord_te} మహాదశ</div>
              ...
              {activeTab === "ruling" && (
                <div className="tab-content">
                  <div style={{ fontSize: 10, color: "var(--accent)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "0.75rem" }}>రూలింగ్ గ్రహాలు · {workspaceData.ruling_planets.query_time}</div>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem", flexWrap: "wrap", gap: 8 }}>
                    ...
                    <div style={{ display: "flex", background: "var(--surface2)", borderRadius: 6, border: "0.5px solid var(--border2)", overflow: "hidden" }}>
                      {([["telugu_english", "తె+EN"], ["english", "EN"]] as const).map(([val, label]) => (
                        <button key={val} onClick={() => setAnalysisLang(val)}
                          style={{ padding: "5px 12px", background: analysisLang === val ? "rgba(201,169,110,0.15)" : "transparent", color: analysisLang === val ? "var(--accent)" : "var(--muted)", border: "none", cursor: "pointer", fontSize: 11, fontFamily: "inherit" }}>{label}</button>
                      ))}
                    </div>
                  </div>
            ...
            {chartData && (
              <div style={{ marginBottom: "0.75rem", flexShrink: 0 }}>
                <button onClick={() => setShowChartDetails(!showChartDetails)} style={{ width: "100%", background: "var(--surface)", border: "0.5px solid var(--border2)", borderRadius: showChartDetails ? "8px 8px 0 0" : 8, padding: "0.75rem 1.25rem", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", color: "var(--muted)", fontSize: 12, letterSpacing: "0.04em" }}>
                  <span style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ color: "var(--accent)", fontSize: 14 }}>◈</span>Chart Details — Planets · Cusps · Dasha · Significators</span>
                ...
                {showChartDetails && (
                  <div style={{ background: "var(--surface)", border: "0.5px solid var(--border2)", borderTop: "none", borderRadius: "0 0 8px 8px", padding: "1.25rem", overflowX: "auto", maxHeight: "50vh", overflowY: "auto" }}>
                    <div style={{ marginBottom: "1.5rem" }}>
                      ...
                      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                        <thead><tr>{["Planet", "Sign", "Degree", "Nakshatra", "Star Lord", "Sub Lord"].map(h => <th key={h} style={{ textAlign: "left", padding: "6px 10px", color: "var(--muted)", fontSize: 10, letterSpacing: "0.06em", textTransform: "uppercase", borderBottom: "0.5px solid var(--border)", fontWeight: 400 }}>{h}</th>)}</tr></thead>
                        <tbody>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                      <div>
                        ...
                        {chartData.dashas?.current_mahadasha && (
                          <div style={{ background: "var(--surface2)", borderRadius: 8, padding: "0.875rem", border: "0.5px solid var(--border)" }}>
                            <div style={{ fontSize: 10, color: "var(--muted)", textTransform: "uppercase" }}>Mahadasha</div>
                            ...
                            {chartData.dashas.current_antardasha && (
                              <div style={{ paddingTop: "0.5rem", borderTop: "0.5px solid var(--border)", marginTop: "0.5rem" }}>
                                <div style={{ fontSize: 10, color: "var(--muted)", textTransform: "uppercase" }}>Antardasha</div>
                      ...
                      <div>
                        <div style={{ fontSize: 10, color: "var(--accent)", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: "0.75rem" }}>Ruling Planets</div>
                        {chartData.ruling_planets && (
                          <div style={{ background: "var(--surface2)", borderRadius: 8, padding: "0.875rem", border: "0.5px solid var(--border)" }}>
                            {[{ label: "Day Lord", value: chartData.ruling_planets.day_lord }, { label: "Lagna Lord", value: chartData.ruling_planets.lagna_sign_lord }, { label: "Moon Star Lord", value: chartData.ruling_planets.moon_star_lord }].map(item => (
                            ...
                            <div style={{ marginTop: 6, display: "flex", gap: 4, flexWrap: "wrap" }}>
                              {chartData.ruling_planets.ruling_planets?.map((p: string) => <span key={p} style={{ background: "rgba(201,169,110,0.1)", color: "var(--accent)", border: "0.5px solid rgba(201,169,110,0.2)", borderRadius: 4, padding: "2px 8px", fontSize: 11 }}>{p}</span>)}
                            </div>
                          </div>
                        )}
                      </div>
